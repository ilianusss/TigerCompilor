...
namespace ast
{
  ...
  template <typename T> inline void DumperDot::dump_def(const T& e) const
  {
    ...
    // FIXED
    if constexpr (requires { e.def_get(); })
      d = e.def_get();
    if (!d)
      return;
  ...
  // FIXED: I added this even if it wasn't required
  inline void DumperDot::node_html_escapable_field(const VarDec& e)
  {
    std::string color = e.is_escapable_get() ? "green" : "red";
    std::string text = e.is_escapable_get() ? "escapable" : "escapable";
    if (inner_fields++)
      ostr_ << misc::iendl;
    ostr_ << "<td port='escapable' bgcolor='" << color << "'>" << text
          << "</td>";
  }
